...
try {
    // ✅ Verificar extensión GD antes de continuar
    if (!extension_loaded('gd')) {
        file_put_contents($carpetaDebug . '/debug_log.txt', "⚠️ Extensión GD no disponible - QR desactivado\n", FILE_APPEND);
    }
    ...
    class ControladorVehiculo {
        ...
        // ✅ CORREGIDO: generarQR ahora verifica GD y no lanza excepción fatal
        private function generarQR(int $idVehiculo, string $tipo, string $placa, string $descripcion): ?string {
            try {
                ...
                if (!extension_loaded('gd')) {
                    file_put_contents($this->carpetaDebug . '/debug_log.txt', "⚠️ GD no disponible, QR no generado\n", FILE_APPEND);
                    return null;
                }

                // ✅ Verificar que imagecreate exista
                if (!function_exists('imagecreate')) {
                    file_put_contents($this->carpetaDebug . '/debug_log.txt', "⚠️ imagecreate no existe, QR no generado\n", FILE_APPEND);
                    return null;
                }
                ...
                if (!file_exists($rutaCarpeta)) {
                    if (!mkdir($rutaCarpeta, 0777, true)) {
                        file_put_contents($this->carpetaDebug . '/debug_log.txt', "⚠️ No se pudo crear carpeta QR: $rutaCarpeta\n", FILE_APPEND);
                        return null;
                    }
                }
                ...
                $contenidoQR  = "VEHICULO\n";
                $contenidoQR .= "Placa: $placa\n";
                $contenidoQR .= "Tipo: $tipo\n";
                $contenidoQR .= "Descripcion: $descripcion\n";
                $contenidoQR .= "Fecha: " . date('Y-m-d H:i:s');

                // ✅ Suprimir deprecated de phpqrcode con @
                @QRcode::png($contenidoQR, $rutaCompleta, QR_ECLEVEL_H, 8);

                if (!file_exists($rutaCompleta)) {
                    file_put_contents($this->carpetaDebug . '/debug_log.txt', "⚠️ El archivo QR no se creó en: $rutaCompleta\n", FILE_APPEND);
                    return null;
                }

                file_put_contents($this->carpetaDebug . '/debug_log.txt', "✅ QR generado: $nombreArchivo\n", FILE_APPEND);
        ...
        public function registrarVehiculo(array $datos): array {
            ...
            try {
                ...
                if ($placaExiste['existe']) {
                    ...
                    return ['success' => false, 'message' => "La placa '{$PlacaVehiculo}' ya esta registrada para un vehiculo tipo '{$v['TipoVehiculo']}'."];
                }

                $tarjetaExiste = $this->modelo->existeTarjetaPropiedad($TarjetaPropiedad);
                if ($tarjetaExiste['existe']) {
                    $v = $tarjetaExiste['vehiculo'];
                    return ['success' => false, 'message' => "La tarjeta '{$TarjetaPropiedad}' ya esta registrada para un vehiculo tipo '{$v['TipoVehiculo']}'."];
                }
                ...
                if ($resultado['success']) {
                    ...
                    // ✅ QR es opcional — si falla no rompe el registro
                ...
                file_put_contents($this->carpetaDebug . '/debug_log.txt', "ERROR registrarVehiculo: " . $e->getMessage() . "\n", FILE_APPEND);
        ...
        public function actualizarVehiculo(int $id, array $datos): array {
            try {
                ...
                if ($resultado['success']) {
                    // ✅ QR es opcional — si falla no rompe la actualización
                ...
                file_put_contents($this->carpetaDebug . '/debug_log.txt', "ERROR actualizarVehiculo: " . $e->getMessage() . "\n", FILE_APPEND);
                return ['success' => false, 'message' => 'Error: ' . $e->getMessage()];
            }
        }
        ...
        public function cambiarEstadoVehiculo(int $id, string $nuevoEstado): array {
            try {
                ...
                if ($resultado['success']) {
                    ...
                    return [
                        'success'     => true,
                        'message'     => "Vehículo $msg correctamente",
                        'nuevoEstado' => $nuevoEstado
                    ];
                }
                return ['success' => false, 'message' => 'Error al cambiar el estado del vehículo'];
            } catch (Exception $e) {
                file_put_contents($this->carpetaDebug . '/debug_log.txt', "ERROR cambiarEstado: " . $e->getMessage() . "\n", FILE_APPEND);
                return ['success' => false, 'message' => 'Error: ' . $e->getMessage()];
            }
        }
        ...
        public function enviarQRPorCorreo(int $idVehiculo, string $correoManual = ''): array {
            ...
            try {
                ...
                if (!file_exists($rutaPHPMailer) || !file_exists($rutaSMTP) || !file_exists($rutaException))
                    throw new Exception('Librerías PHPMailer no encontradas en: ' . dirname($rutaPHPMailer));
                ...
                <div class='container'>
                    ...
                    <div class='content'>
                        ...
                        <p><strong>Importante:</strong> Guarda este código en un lugar seguro.</p>
                    </div>
                    <div class='footer'>
                ...
                file_put_contents($this->carpetaDebug . '/debug_log.txt', "ERROR enviarQR: " . $e->getMessage() . "\n", FILE_APPEND);
                return ['success' => false, 'message' => $e->getMessage()];
            }
        }

        // ── Obtener tipo propietario ──────────────────────────────────────────
        public function obtenerTipoPropietario(int $idVehiculo): array {
            try {
                ...
                file_put_contents($this->carpetaDebug . '/debug_log.txt', "ERROR obtenerTipoPropietario: " . $e->getMessage() . "\n", FILE_APPEND);
                return ['success' => false, 'message' => $e->getMessage()];
            }
        }
    }
    ...
    if ($accion === 'registrar') {
        ...
        $resultado = $id > 0
            ? $controlador->actualizarVehiculo($id, [
