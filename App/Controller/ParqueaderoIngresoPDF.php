...
class ParqueaderoIngresoPDF extends FPDF {
    ...
    function TablaVehiculos($vehiculos) {
        ...
        // Anchos de columna optimizados para vehículos
        $w = array(25, 25, 30, 40, 30, 40);
        $headers = array('Código QR', 'Placa', 'Tipo', 'Descripción', 'Movimiento', 'Fecha');
        ...
        foreach($vehiculos as $row) {
            ...
            // Código QR
            $this->Cell($w[0], 8, utf8_decode(substr($row['QrVehiculo'], 0, 10)), 'LR', 0, 'C', true);
            
            // Placa
            $this->Cell($w[1], 8, utf8_decode($row['PlacaVehiculo']), 'LR', 0, 'C', true);
            
            // Tipo de vehículo
            $this->Cell($w[2], 8, utf8_decode(substr($row['TipoVehiculo'], 0, 12)), 'LR', 0, 'C', true);
            
            // Descripción
            $this->Cell($w[3], 8, utf8_decode(substr($row['DescripcionVehiculo'], 0, 18)), 'LR', 0, 'L', true);
            ...
            $this->Cell($w[4], 8, utf8_decode(substr($tipoMov, 0, 12)), 0, 0, 'C');
            $this->SetTextColor($colorTexto[0], $colorTexto[1], $colorTexto[2]);
            
            // Fecha de ingreso
            $this->Cell($w[5], 8, $row['FechaIngreso'], 'LR', 1, 'C', false);
...
if (isset($_GET['accion']) && $_GET['accion'] === 'pdf') {
    $controller = new ParqueaderoIngresoPDF();
