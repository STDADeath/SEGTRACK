document.addEventListener('DOMContentLoaded', function () {
    ...
    function cargarVisitantes() {
        ...
        .then(res => {
            ...
            if (!Array.isArray(res) || res.length === 0) {
                ...
                if (msgDiv) {
                    msgDiv.textContent = 'No hay visitantes activos registrados.';
                    msgDiv.className   = 'form-text text-warning';
                }
                return;
            }
    ...
    // ══════════════════════════════════════════════
    // 4. CARGAR DISPOSITIVOS (filtra por visitante)
    // ══════════════════════════════════════════════
    function cargarDispositivos(idVisitante = '') {
        ...
        .then(res => {
            ...
            if (!Array.isArray(res) || res.length === 0) {
                ...
                if (msgDiv) {
                    msgDiv.textContent = 'No hay dispositivos activos para este visitante.';
                    msgDiv.className   = 'form-text text-warning';
                }
                return;
            }
    ...
    // cuando cambia el visitante → recargar dispositivos de ese visitante
    ...
    // Novedades: mínimo 10 caracteres
    ...
    // PDF: validar tipo y tamaño, mostrar nombre
    ...
    // ══════════════════════════════════════════════
    // 5. CAMPOS CONDICIONALES (visitante / dispositivo)
    // ══════════════════════════════════════════════
    ...
    if (selTraeDispositivo) {
        selTraeDispositivo.addEventListener('change', function () {
            ...
            if (!trae) {
                ...
                // Cargar dispositivos del visitante seleccionado
    ...
    if (form) {
        form.addEventListener('submit', async function (e) {
            ...
            if (!turno) {
                ...
                document.getElementById('TurnoBitacora').classList.add('is-invalid');
                return;
            }
            ...
            if (fechaSeleccionada < inicioHoy) {
                ...
                document.getElementById('FechaBitacora').classList.add('is-invalid');
                return;
            }
            ...
            if (tieneVisit === 'si') {
                if (!idVisitante) {
                    ...
                    document.getElementById('IdVisitante').classList.add('is-invalid');
                    return;
                }
                if (!traeDisp) {
                    Swal.fire({ icon: 'error', title: 'Campo requerido', text: 'Debe indicar si el visitante trae dispositivo', confirmButtonColor: '#e74a3b' });
                    document.getElementById('TraeDispositivo').classList.add('is-invalid');
                    return;
                }
                if (traeDisp === 'si' && !idDisp) {
                    ...
                    document.getElementById('IdDispositivo').classList.add('is-invalid');
                    return;
                }
            }
            ...
            if (archivoPDF) {
                if (archivoPDF.type !== 'application/pdf') {
                    Swal.fire({ icon: 'error', title: 'Archivo inválido', text: 'El archivo adjunto debe ser un PDF', confirmButtonColor: '#e74a3b' });
                    return;
                }
            ...
            try {
                const response = await fetch('../../Controller/ControladorBitacora.php', {
                    method: 'POST',
                    body:   formData
                });
                ...
                if (data.success) {
                    Swal.fire({
                        ...
                        const n = new Date();
                        if (inputFecha) {
                            inputFecha.value = `${n.getFullYear()}-${pad(n.getMonth()+1)}-${pad(n.getDate())}T${pad(n.getHours())}:${pad(n.getMinutes())}`;
                        }
